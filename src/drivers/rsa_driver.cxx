...
std::string RSADriver::get_name() {
    return "ssh-rsa";
}

std::pair<std::string, std::string> 
RSADriver::import_auth_key(const std::string &file) {
...
bool RSADriver::DSA_verify(const std::string &vk, const std::string &message, const std::string signature) {
...
std::string RSADriver::get_sign_pubkey_blob_from_file(const std::vector<unsigned char> &data, size_t &idx) {
    uint32_t len;
...
std::string RSADriver::get_sign_privkey_blob_from_file(
...
std::string RSADriver::get_sign_pubkey(
        const std::vector<unsigned char> &data, size_t &idx) {
    uint32_t len;
...
void RSADriver::put_sign_pubkey(
...
std::string RSADriver::get_signature(
        const std::vector<unsigned char> &data, size_t &idx) {
    uint32_t len;
    get_integer_big(&len, data, idx);
    auto algo_name = get_string(data, idx);
